...
class DirectmediaDecompressor:
    ...
    def decompress_section(self, data: bytes, compression_type: CompressionType) -> bytes | None:
    ...
    def _decompress_packbits(self, data: bytes) -> bytes | None:
    ...
    def analyze_text_dki_section(self, data: bytes) -> dict[str, Any]:
        ...
        analysis = {"length": len(data), "patterns_found": [], "possible_strings": [], "structure_hints": []}
    ...
    def _extract_text_from_record(self, record_data: bytes) -> str | None:
    ...
    def _decompress_huffman(self, data: bytes) -> bytes | None:
    ...
    def extract_text_content(self, file_path: Path, max_sections: int = 10) -> dict[str, Any]:
        ...
        result = {"analysis": analysis, "extracted_sections": [], "total_extracted_size": 0, "errors": []}
                    ...
                    if 2 <= length_byte <= 50:  # Reasonable German word/phrase length
                        ...
                        if text_end <= len(file_data):
                            ...
                            try:
                                ...
                                if (
                                    all(c in valid_chars for c in text)
                                    and any(c.isalpha() for c in text)  # Must contain letters
                                    and len(text.strip()) >= 2  # Minimum length
                        ...
                        result["extracted_sections"].append(
                            {
                                ...
                                "records": [
                                    ...
                                    for record in section_records[:20]
                                ],  # First 20 records
                            }
                        )
                ...
                if 5 <= record_length <= 5000:  # Reasonable text length for philosophical content
                    ...
                    if text_end <= len(file_data):
                        ...
                        try:
                            ...
                            if (
                                len(text.strip()) >= 10  # Substantial content
                                and any(c.isalpha() for c in text)  # Contains letters
                                and not text.isdigit()  # Not just numbers
                                and text.count(" ") > 2  # Contains sentences/paragraphs
                                and not all(c in ".,;:!?- " for c in text)
                ...
                if data[i : i + 2] == b"\x78\x9c":  # Zlib signature
                    ...
                    try:
                        ...
                        if len(text) > 100 and any(
                            word in text.lower() for word in ["philosoph", "aristotel", "metaphysik", "ethik"]
                        ):
    ...
    def _extract_readable_text_blocks(
        self, data: bytes, max_blocks: int = 100, min_length: int = 20
    ) -> list[dict[str, Any]]:
                    ...
                    try:
                        ...
                        if self._validate_text_content(text, min_length):
                            ...
                            if len(clean_text) >= min_length:
                                text_blocks.append({"offset": start, "length": sequence_length, "text": clean_text})
                    except:
                        pass
            else:
                i += 1
